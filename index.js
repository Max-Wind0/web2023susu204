...
const getPost = (post, users) => {
  ...
  newPost.classList =
    `container px-4 py-4 bg-blue-900 border-4 border-blue-200 justify-self-center w-[90%] md:w-full card`;
  ...
  editButton.classList = 'active:text-purple-300 active:scale-90 edit-button';
...
const addPostsToDOM = async () => {
  ...
  posts = await fetch('posts.json').then(
    (res) => res.json()
  );

  posts.forEach((post) => {
    postsList.insertBefore(getPost(post, users), postsList.lastChild);
  });
...
function closeModal(e) {
  ...
  e.target.setAttribute('data-post-id', '');
}
